...
test('reorg tests', (t) => {
  t.test('should correctly reorg the chain if the total difficulty is higher on a lower block number than the current head block', async (st) => {
    ...
    while (TD_High.cmp(TD_Low) == -1) {
      ...
      blocks_highTD.push(generateConsecutiveBlock(blocks_highTD[blocks_highTD.length - 1] || genesis, 1))
